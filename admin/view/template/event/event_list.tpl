          ...
          <tbody>
            ...
            <tr>
              <td style="text-align: center;"><?php if ($event['selected']) { ?>
                ...
                <?php } ?></td>
              <td class="center"><img src="<?php echo $event['image']; ?>" alt="<?php echo $event['title']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /></td>
              <td class="left"><?php echo $event['title']; ?></td>
              <td class="left"><?php echo $event['status']; ?></td>
...
$('input[name=\'filter_title\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=event/event/autocomplete&token=<?php echo $token; ?>&filter_title=' +  encodeURIComponent(request.term),
			dataType: 'json',
